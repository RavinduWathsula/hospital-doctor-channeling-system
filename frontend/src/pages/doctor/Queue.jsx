        ...
        <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-500">
            <div>
                <h1 className="text-2xl font-bold text-gray-800 flex items-center">
                    <Users className="mr-3 text-theme-600" size={28} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white rounded-2xl shadow-sm border border-green-100 p-8 relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-green-50 rounded-bl-full -mr-16 -mt-16 z-0"></div>
                        <div className="relative z-10">
                            <h2 className="text-sm font-bold text-green-600 uppercase tracking-wider mb-4">Current Consultation</h2>
                            
                            {queueData.currentPatient ? (
                                <div className="flex flex-col md:flex-row items-center gap-6">
                                    <div className="w-24 h-24 bg-gradient-to-br from-green-400 to-green-600 text-white rounded-2xl flex flex-col items-center justify-center shadow-lg transform rotate-3">
                                ...
                                <div className="text-center py-10 bg-green-50/50 rounded-xl border border-dashed border-green-200">
                                    <UserPlus className="mx-auto text-green-300 mb-3" size={48} />
                                    <h3 className="text-lg font-semibold text-green-700">No Active Consultation</h3>
                                    <p className="text-green-600/70 mt-1">Ready for the next patient.</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-amber-100 p-6">
                        <h2 className="text-sm font-bold text-amber-600 uppercase tracking-wider mb-4 flex items-center">
                            <ArrowRight className="mr-2" size={16} /> Next in Line
                        </h2>
                        {queueData.nextPatient ? (
                            <div className="flex items-center justify-between p-4 bg-amber-50 rounded-xl border border-amber-200">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 bg-amber-100 text-amber-700 rounded-lg flex items-center justify-center font-bold text-lg">
                                        #{queueData.nextPatient.queue_number}
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-amber-900">{queueData.nextPatient.first_name} {queueData.nextPatient.last_name}</h3>
                                        <p className="text-sm text-amber-700">Scheduled: {queueData.nextPatient.appointment_time}</p>
                                    </div>
                                </div>
                                <button 
                                    onClick={() => handleStatusUpdate(queueData.nextPatient.id, 'CALLED')}
                                    disabled={!!queueData.currentPatient}
                                    className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed text-white rounded-xl font-bold transition-colors shadow-sm flex items-center"
                                >
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-blue-100 flex flex-col h-[600px]">
                    <div className="p-6 border-b border-blue-100 bg-blue-50/30 rounded-t-2xl flex justify-between items-center">
                        <h2 className="text-lg font-bold text-blue-900">Waiting Room</h2>
                        ...
                        {queueData.waitingPatients && queueData.waitingPatients.length > 0 ? (
                            <div className="space-y-2">
                                {queueData.waitingPatients.map(patient => (
                                    <div key={patient.id} className="p-4 flex items-center gap-4 bg-white hover:bg-blue-50 border border-transparent hover:border-blue-100 rounded-xl transition-colors">
                                        <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center font-bold text-sm border border-blue-200">
                                            {patient.queue_number}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="font-semibold text-blue-900 truncate">{patient.first_name} {patient.last_name}</p>
                                            <p className="text-xs text-blue-600/70">{patient.appointment_time}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
